...
admin/index.php         панель: вход, список заявок, статусы
...
Открывается по адресу `твойдомен.ru/admin/`. Внутри: сводка,
список заявок, фильтр по статусам и переключение статуса
(новая → в работе → успех / отказ).

**Статистика Метрики прямо в админке** — необязательный пункт.
Нужен OAuth-токен: создать приложение на https://oauth.yandex.ru
с правами на Метрику, получить токен и вписать в `metrika.token`.
Без него админка работает, просто без блока с визитами.

### Что важно знать

- Форма отправляет заявку на `/api/lead.php` и **параллельно** открывает
  WhatsApp. Даже если человек передумает писать в WhatsApp, заявка уже в базе.
- Если бэкенда нет (например, сайт лежит на GitHub Pages) — запрос молча
  не проходит, а WhatsApp открывается как раньше. Сайт не ломается.
...
Секция «Сколько это будет стоить» перед блоком «Как мы работаем».
Формат и число гостей → порядок суммы, кнопка переносит расчёт в форму заявки.

Считается так: `число позиций на гостя × средняя цена позиции × гостей + доплаты`.

- **Средняя цена позиции — 150 ₽.** Выведена из реального меню на Menusa:
  тарталетки 4000/36 = 111 ₽, «Царские» 5500/36 = 153 ₽, жульены 3200/16 = 200 ₽,
  тирамису 4000/24 = 167 ₽, штучные канапе 120–160 ₽. Константа `PIECE_PRICE`
  в `js/main.js`.
- **Позиций на гостя** — 4 для кофе-брейка, 8 для фуршета, 12 для банкета,
  10 для гастробоксов. Это отраслевая норма, а не данные из брифа.
  Задаётся в `data-pieces` у кнопок формата в разметке.
- **Доплаты** — реальные цены аренды: накрытие 10 000 ₽, кофемашина 5 000 ₽.

Обе цифры стоит показать Екатерине: цена позиции взята из её же меню,
а норма на гостя — общая, и под её порции может отличаться.

## Частые вопросы

Семь вопросов с ответами перед блоком контактов, раскрываются на `<details>` —
без скриптов. Плюс микроразметка `FAQPage`: Яндекс и Google умеют показывать
такие ответы прямо в выдаче.

Ответы написаны только по тому, что есть в брифе и меню: география, масштаб,
что входит в стоимость, работа с юрлицами, сборка меню, халяльный бокс,
часы работы. **Про сроки бронирования, предоплату, дегустацию и способы оплаты
вопросов нет — этих данных у меня не было**, выдумывать не стал.
Если Екатерина ответит, добавлю.
...
Собран из отдельных слоёв. Сначала там лежал готовый баннер одной картинкой,
но его исходник был 1280×720: на мониторах шире этого вшитые надписи
растягивались и мылили, а апскейл не помогает — из 1280 пикселей деталей
не возьмётся.

Теперь каждый слой — своя картинка, и все они **уменьшаются**, а не тянутся:

| Файл | Что это | Исходник | На экране 1920 |
|---|---|---|---|
| `hero-scene.*` | размытый зал, фон | 1920×1080 | по размеру |
| `hero-figure.*` | Екатерина без фона | 843×1303 | уменьшение 1,8× |
| `hero-title.*` | логотип и подзаголовок | 1360×344 | уменьшение 2,1× |
| `hero-lead.*` | две строки подводки | 1138×113 | уменьшение 2,1× |

Кнопка «Выбрать меню» — настоящая, не нарисованная.

Высота первого экрана задана **определённой** (`height`, не `min-height`):
фигура меряется по высоте (`height:100%`), и без явной высоты она растянула бы
блок своим натуральным размером в 1303 пикселя.

На телефоне вырезка заметно шире прежней и вдвоём с текстом в 375 пикселей
не помещается, поэтому она заходит за левый край. Подводка там идёт настоящим
текстом (`.hero__lead-text`): картинкой на такой ширине её было бы не прочитать.
Оба варианта в разметке, переключаются через `display`.

Исходники слоёв лежат в папке `без фона чтобы собрать главный жкран/`,
она в `.gitignore` и на хостинг не уходит.
