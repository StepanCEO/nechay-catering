...
| `assets/img/hero-bg.*` | фон первого экрана, десктоп | кадр зала из её съёмки |
| `assets/img/hero-mobile.*` | фон первого экрана, телефон | тот же кадр, вертикальный |
...
Обычный пароль в конфиг писать нельзя, только хэш. Создай файл `hash.php`
в корне сайта с таким содержимым, открой его в браузере, скопируй строку
в `admin_password_hash`, **и сразу удали файл**:

```php
<?php echo password_hash('придумай-пароль', PASSWORD_DEFAULT);
```
...
Собран из слоёв, а не одной картинкой. Сначала там лежал готовый баннер
`главный.jpg`, но его исходник — 1280×720, и на мониторах шире этого
вшитые в картинку надписи расплывались. Апскейл не помогает: из 1280
пикселей детали не возьмутся.

Теперь фотографией остался только фон, всё остальное — отдельные слои:

| Слой | Что это | Почему чётко |
|---|---|---|
| фон | `hero-bg.*` на десктопе, `hero-mobile.*` на телефоне | размытый, резкость на нём не видна |
| Екатерина | `chef.png`, вырезка | 464 px в ширину, показывается уже |
| логотип | `logo-light.png` | 656 px, рисуется в 326–461 px, то есть с уменьшением |
| надписи | обычный HTML-текст | векторные, чёткие при любом разрешении |

Фоны сделаны из её съёмки (кадр банкетного зала), в AVIF и WebP.
Десктопный — 2400×760, мобильный — 900×840, нужная версия выбирается
через `media` в `<picture>`.

Пропорции подобраны под исходный баннер: высота ≈48% от ширины экрана,
Екатерина занимает 9–37% ширины, логотип по центру правой половины.
Всё это в блоке `.hero__live` в `css/style.css`.
...
Первый экран собирается из тех же слоёв, что и на десктопе, только теснее:
Екатерина уже, текст мельче, свой вертикальный фон. Правила — в блоке
«ПЕРВЫЙ ЭКРАН НА ТЕЛЕФОНЕ» в `css/style.css`.
...
Принудительные переносы `<br>` в подводках на телефоне скрываются.
**Если добавляешь такой перенос — ставь пробел сразу после него**,
иначе при скрытом переносе слова склеятся.
