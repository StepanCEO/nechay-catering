...
admin/check.php         проверка подключения Метрики, только из-под входа
...
Если цифры не появились, есть страница проверки — `твойдомен.ru/admin/check.php`
(ссылка на неё есть и в самой инструкции на вкладке «Аналитика»). Она
показывает, какой именно `config.php` прочитал PHP, когда файл менялся
в последний раз, задан ли токен и что отвечает API. Токен целиком не
печатает — только длину и первые символы. Открывается только из-под
входа в админку.
