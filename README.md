...
Лежат в `assets/img/clients/`. Знаки очень разные по цвету, поэтому в вёрстке
они приглушены до общего серого тона (`filter: grayscale`), а цвет возвращается
по наведению — иначе ряд рассыпается на пёструю мозаику.
...
**Сбербанк и МСП Банк набраны текстом — файлов найти не удалось.**
Сайт Сбера отдаёт заглушку вместо страницы, сайт МСП Банка снаружи недоступен,
а на Wikimedia их логотипов нет. Если Екатерина попросит у них фирменные файлы,
достаточно положить `sberbank.svg` и `mspbank.svg` в ту же папку и заменить
`<span>` на `<img class="clients__logo">` в `index.html`.
...
> от компаний лучше получить — заодно они пришлют официальные файлы логотипов.
