...
Счётчик и цели уже в коде, ждут номер. Как включить:

1. Завести счётчик на https://metrika.yandex.ru для адреса сайта
2. Скопировать его номер (8 цифр)
3. Вставить в константу `METRIKA_ID` в начале `js/main.js`
4. Поднять версию `?v=` у скрипта в обеих страницах

Пока строка пустая — Метрика не грузится вообще, лишних запросов нет.

**Цели уже расставлены**, в Метрике их надо создать с такими же
идентификаторами (тип «JavaScript-событие»):
...
Тогда в Метрике будет видно не просто «зашло 40 человек», а сколько
из них написали, позвонили и открыли меню — и откуда они пришли.
